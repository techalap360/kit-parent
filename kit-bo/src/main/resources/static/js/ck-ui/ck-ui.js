...
ck.ui.config.editmode = function(){
	...
	if($('a.mainform-btn.confirm-btn').length > 0){
		$('a.mainform-btn.confirm-btn').off('click').on('click', function(e){
			e.preventDefault();
			submitMainForm();
		});
	}
	...
	if($('a.mainform-btn.archive-btn').length > 0){
		$('a.mainform-btn.archive-btn').off('click').on('click', function(e){
			e.preventDefault();
	...
	// Bind archive button event
	if($('a.mainform-btn.restore-btn').length > 0){
		$('a.mainform-btn.restore-btn').off('click').on('click', function(e){
			e.preventDefault();
			submitMainForm($(this).attr('href'));
		});
	}
